...
def test_load_excel_no_file():
    with pytest.raises(FileNotFoundError):
        load_excel("data/magic.xlsx")

def test_load_excel_value_error():
    with pytest.raises(ValueError):
        load_excel("data/catalogue.xlsx",0,engine="notrealxml")

@pytest.mark.skip("Evil Excel Handles These Errors")
def test_load_excel_read(tmp_path):
    import shutil
    shutil.copy("data/catalogue.xlsx", tmp_path / "bad.xlsx")
    with open(tmp_path / "bad.xlsx", "ab") as f:
        f.write("GarbageDataHereHAHAHAHA</tag>".encode())
    with pytest.raises(ValueError):
        load_excel(tmp_path / "bad.xlsx")